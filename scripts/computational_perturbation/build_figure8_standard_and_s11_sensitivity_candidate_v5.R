...
if (length(args) != 2L) {
  stop(
    "Usage: Rscript build_figure8_standard_and_s11_sensitivity_candidate_v5.R ",
...
main_stem <- file.path(
  figure_dir,
  "Figure_8_standard_scTenifoldKnk_network_responses_pvalue_format_20260726_v12"
)
supp_stem <- file.path(
  figure_dir,
  "Supplementary_Figure_S11_matched_control_sensitivity_pvalue_format_20260726_v6"
)
...
panel_b <- ggplot(
  ...
  labs(
    ...
    subtitle = expression(atop(
      ...
      paste("dots: BH-adjusted ", italic(P), " < 0.05")
    )),
    x = NULL, y = NULL
  ) +
  theme_pub +
...
panel_c <- ggplot(
  ...
  labs(
    ...
    subtitle = expression(
      paste("Dashed: BH-adjusted ", italic(P), " = 0.05; gold: GSK3B")
    ),
    x = "Response-gene rank",
    y = expression(-log[10](paste("BH-adjusted ", italic(P))))
  ) +
  theme_pub +
  theme(
...
panel_d <- ggplot(candidate_endpoints, aes(y = candidate)) +
  ...
  labs(
    ...
    x = expression(
      paste("Response genes at BH-adjusted ", italic(P), " < 0.05 (log1p axis)")
    ),
    y = NULL
  ) +
  theme_pub
