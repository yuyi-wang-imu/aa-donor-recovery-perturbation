...
def find_one(pattern: str) -> Path:
    ...
    if len(hits) != 1:
        raise RuntimeError(f"Expected exactly one {pattern}; observed {len(hits)}: {hits}")
    return hits[0]
...
def main() -> int:
    ...
    for gene, candidate, role, frozen_order in selected:
        ...
        for donor, axis in axes.items():
            ...
            deleted = [[t for t in sequences[i] if t != token] for i in donor_cells]
            deleted_emb = embed_sequences(model, deleted, pad_token, device, batch_size=16)
            original_emb = embeddings[donor_cells]
            shifts = (deleted_emb - original_emb) @ axis
            donor_rows.append({"gene": gene, "candidate": candidate, "run_role": role, "subject": donor, "n_cells": len(donor_cells), "mean_deletion_recovery_shift": float(np.mean(shifts)), "median_deletion_recovery_shift": float(np.median(shifts))})
    ...
    gene_table = donor_table.groupby(["gene", "candidate", "run_role"], as_index=False).agg(
        ...
        mean_deletion_recovery_shift=("mean_deletion_recovery_shift", "mean"),
        median_deletion_recovery_shift=("mean_deletion_recovery_shift", "median"),
        positive_donor_fraction=("mean_deletion_recovery_shift", lambda v: float(np.mean(np.asarray(v) > 0))),
    )
    ...
    for gene, subset in donor_table.groupby("gene", sort=False):
        summary = bootstrap_summary(subset["mean_deletion_recovery_shift"].to_numpy(float), rng)
    ...
    for target in MVP_TARGETS:
        ...
        if target_row.empty or len(controls) < 4:
            ...
            continue
        effect = float(target_row.iloc[0]["mean_deletion_recovery_shift"])
        null = controls["mean_deletion_recovery_shift"].to_numpy(float)
    ...
    full_rank = donor_table.groupby("gene")["mean_deletion_recovery_shift"].mean().sort_values(ascending=False)
    lodo_rows = []
    for donor in sorted(donor_table.subject.unique()):
        leave = donor_table[donor_table.subject != donor].groupby("gene")["mean_deletion_recovery_shift"].mean()
    ...
    gate = {
        "decision_scope": "engineering_MVP_only_not_formal_significance",
    ...
    qc = {
        ...
        "all_gene_effects_finite": bool(np.isfinite(donor_table.mean_deletion_recovery_shift).all()),
    }
    ...
    execution = {
        ...
        "source_note": "One gene deleted at a time; no grouped candidate deletion; no endpoint/threshold/candidate tuning.",
    }
