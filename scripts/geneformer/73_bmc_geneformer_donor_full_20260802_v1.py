...
ENGINE = Path(__file__).with_name("71_bmc_geneformer_donor_mvp_20260802_v1.py")
spec = importlib.util.spec_from_file_location("bmc_geneformer_engine_v1", ENGINE)
...
def stratified_null(effects: pd.DataFrame, candidates: list[str], reps: int = 100000, seed: int = 20260802):
    ...
    for candidate in candidates:
        ...
        if cand.empty or ctrl.empty:
            ...
            continue
        value = float(cand.iloc[0].mean_deletion_recovery_shift)
        null = ctrl.mean_deletion_recovery_shift.to_numpy(float)
...
def sctenifold_consistency(effects: pd.DataFrame, candidates: list[str]):
    ...
    for candidate in candidates:
        ...
        records.append({
            "candidate": candidate,
            "geneformer_signed_deletion_shift": float(cand.iloc[0].mean_deletion_recovery_shift),
            "geneformer_absolute_deletion_shift": abs(float(cand.iloc[0].mean_deletion_recovery_shift)),
    ...
    if len(table) >= 3:
        summary = {
            ...
            "spearman_signed_vs_response_count": float(spearmanr(table.geneformer_signed_deletion_shift, table.sctenifold_significant_response_genes).statistic),
            "spearman_absolute_vs_response_count": float(spearmanr(table.geneformer_absolute_deletion_shift, table.sctenifold_significant_response_genes).statistic),
            "spearman_absolute_vs_max_abs_Z": float(spearmanr(table.geneformer_absolute_deletion_shift, table.sctenifold_max_abs_Z).statistic),
        }
...
def main() -> int:
    ...
    full_gate = {
        "decision_scope": "full_10_candidate_plus_frozen_matched_controls",
    ...
    full_gate["next_step"] = "FREEZE_FULL_RESULTS_AND_BUILD_FIGURE9_CANDIDATE" if full_gate["technical_pass"] else "STOP_AND_AUDIT_FULL_COVERAGE"
    (output_dir / "full_analysis_gate.json").write_text(json.dumps(full_gate, indent=2), encoding="utf-8")
    print("FULL_ANALYSIS_COMPLETE")
