...
figure regression references used in the current BMC Pharmacology and
Toxicology submission. The current submission-file byte sizes and SHA-256
values are recorded in `BMC_PHARMACOLOGY_TOXICOLOGY_ASSET_CHECKSUMS.tsv`; the
historical Scientific Reports transfer values remain in
`CURRENT_MANUSCRIPT_ASSET_CHECKSUMS.tsv`. The current submission PNGs are
...
Genomics submission. A visual abstract is optional for the current BMC
Pharmacology and Toxicology submission and has been omitted from its package.
Its image sources and reuse boundaries are documented in
...
The current BMC manuscript contains 103 sequentially cited references;
